function daydiff(first, second) {
  ...
  if((typeof totWidth !== 'undefined') && value < totWidth) {
	value = totWidth;
  } else {
	value = value;
  }
  ...
  function showNewContent(timelineComponents, timelineTotWidth, string) {
	...
	if (string === 'next') {
	  newContent = visibleContent.next();
	} else {
	  newContent = visibleContent.prev();
	}
	...
	if (newContent.length > 0) {
	  ...
	  if (string === 'next') {
		newEvent = selectedDate.parent('li').next('li').children('a');
	  } else {
		newEvent = selectedDate.parent('li').prev('li').children('a');
	  }
	  ...
	  updateTimelinePositions(string, newEvent, timelineComponents);
	}
  }
